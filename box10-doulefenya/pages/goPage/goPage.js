...
Page({
  ...
  onLoad: function (options) {
    //如果是自己创建的活动，需要分享后才能打开，其他人不用(涉嫌诱导分享 去掉算了)
    // if (options.creater){
    //   this.setData({
    //     hasShared : false
    //   });
  ...
  joinGame(){
    if(this.data.hasShared){
      ...
      wxUtils.request('joinGroup',data, successFuc.bind(this), failFuc.bind(this), complete.bind(this),"GET");
    }else{
  ...
  onShareAppMessage: function (e) {
    // if(e.from == 'button'){
      ...
      return {
        title: `${this.data.ac_Name}:你会分在哪个组呢？`,
